...
class NewsViewer extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _NewsViewerState();
}
        ...
        child: Offstage(
          ...
          child: InAppWebView(
              ...
              onLoadStop: (controller, url) async {
                if (url == Uri.parse("https://www.borghidarberia.it/news/")) {
                  var _one = await controller.evaluateJavascript(
                      source:
                          'document.getElementsByClassName("grecaptcha-badge")[0].remove()');
                  var _two = await controller.evaluateJavascript(
                      source: 'document.querySelector("#header-grid").remove();');
                  ...
                  var _four = await controller.evaluateJavascript(
                      source:
                          'document.querySelector("#wp-block-themeisle-blocks-advanced-columns-2ff08d89").remove();');

                  _printHello();
                }
              },
              initialUrlRequest:
                  URLRequest(url: Uri.parse("https://www.borghidarberia.it/news/")),
